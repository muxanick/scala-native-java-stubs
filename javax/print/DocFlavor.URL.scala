package javax.print

import java.lang.{Object, String}
import scala.scalanative.annotation.stub

/** Class DocFlavor.URL provides predefined static constant DocFlavor
 *  objects.
 *  For example doc flavors using a Uniform Resource Locator (java.net.URL) as the print data
 *  representation  class.
 *  
 */
object DocFlavor_URL extends DocFlavor {

    /** Constructs a new doc flavor with the given MIME type and a print
     *  data representation class name of "java.net.URL".
     */
    @stub
    def URL(mimeType: String) = ???

    /** Doc flavor with MIME type =
     *  "application/octet-stream",
     *  print data representation class name = "java.net.URL".
     */
    @stub
    val AUTOSENSE: DocFlavor.URL = ???

    /** Doc flavor with MIME type = "image/gif", print data
     *  representation class name = "java.net.URL".
     */
    @stub
    val GIF: DocFlavor.URL = ???

    /** Doc flavor with MIME type = "image/jpeg", print data
     *  representation class name = "java.net.URL".
     */
    @stub
    val JPEG: DocFlavor.URL = ???

    /** Doc flavor with MIME type = "application/vnd.hp-PCL",
     *  print data representation class name = "java.net.URL".
     */
    @stub
    val PCL: DocFlavor.URL = ???

    /** Doc flavor with MIME type = "application/pdf", print
     *  data representation class name = "java.net.URL".
     */
    @stub
    val PDF: DocFlavor.URL = ???

    /** Doc flavor with MIME type = "image/png", print data
     *  representation class name = "java.net.URL".
     */
    @stub
    val PNG: DocFlavor.URL = ???

    /** Doc flavor with MIME type = "application/postscript",
     *  print data representation class name = "java.net.URL".
     */
    @stub
    val POSTSCRIPT: DocFlavor.URL = ???

    /** Doc flavor with MIME type = "text/html",
     *  encoded in the host platform encoding.
     */
    @stub
    val TEXT_HTML_HOST: DocFlavor.URL = ???

    /** Doc flavor with MIME type =
     *  "text/html; charset=us-ascii",
     *  print data representation class name =
     *  "java.net.URL" (byte stream).
     */
    @stub
    val TEXT_HTML_US_ASCII: DocFlavor.URL = ???

    /** Doc flavor with MIME type =
     *  "text/html; charset=utf-16",
     *  print data representation class name =
     *  "java.net.URL" (byte stream).
     */
    @stub
    val TEXT_HTML_UTF_16: DocFlavor.URL = ???

    /** Doc flavor with MIME type =
     *  "text/html; charset=utf-16be"
     *  (big-endian byte ordering),
     *  print data representation class name =
     *  "java.net.URL" (byte stream).
     */
    @stub
    val TEXT_HTML_UTF_16BE: DocFlavor.URL = ???

    /** Doc flavor with MIME type =
     *  "text/html; charset=utf-16le"
     *  (little-endian byte ordering),
     *  print data representation class name =
     *  "java.net.URL" (byte stream).
     */
    @stub
    val TEXT_HTML_UTF_16LE: DocFlavor.URL = ???

    /** Doc flavor with MIME type =
     *  "text/html; charset=utf-8",
     *  print data representation class name =
     *  "java.net.URL" (byte stream).
     */
    @stub
    val TEXT_HTML_UTF_8: DocFlavor.URL = ???

    /** Doc flavor with MIME type = "text/plain",
     *  encoded in the host platform encoding.
     */
    @stub
    val TEXT_PLAIN_HOST: DocFlavor.URL = ???

    /** Doc flavor with MIME type =
     *  "text/plain; charset=us-ascii",
     *  print data representation class name =
     *  "java.net.URL" (byte stream).
     */
    @stub
    val TEXT_PLAIN_US_ASCII: DocFlavor.URL = ???

    /** Doc flavor with MIME type =
     *  "text/plain; charset=utf-16",
     *  print data representation class name =
     *  java.net.URL"" (byte stream).
     */
    @stub
    val TEXT_PLAIN_UTF_16: DocFlavor.URL = ???

    /** Doc flavor with MIME type =
     *  "text/plain; charset=utf-16be"
     *  (big-endian byte ordering),
     *  print data representation class name =
     *  "java.net.URL" (byte stream).
     */
    @stub
    val TEXT_PLAIN_UTF_16BE: DocFlavor.URL = ???

    /** Doc flavor with MIME type =
     *  "text/plain; charset=utf-16le"
     *  (little-endian byte ordering),
     *  print data representation class name =
     *  "java.net.URL" (byte stream).
     */
    @stub
    val TEXT_PLAIN_UTF_16LE: DocFlavor.URL = ???

    /** Doc flavor with MIME type =
     *  "text/plain; charset=utf-8",
     *  print data representation class name =
     *  "java.net.URL" (byte stream).
     */
    @stub
    val TEXT_PLAIN_UTF_8: DocFlavor.URL = ???
}
