...
@FunctionalInterface trait Comparator[T] {
    ...
    def Comparator[T]: default[U <: Comparable[_ >: U]] = ???

    @stub
    // Returns a lexicographic-order comparator with a function that
    // extracts a key to be compared with the given Comparator.
    def Comparator[T]: default[U] = ???
...
object Comparator {
    ...
    def Comparator[T]: [T, U <: Comparable[_ >: U]] = ???
    ...
    // order.
    def Comparator[T]: [T <: Comparable[_ >: T]] = ???
