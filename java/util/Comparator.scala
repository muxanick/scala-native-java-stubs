...
@FunctionalInterface trait Comparator[T] {
     ...
     */
    @stub
    val Comparator[T]: default[U <: Comparable[_ >: U]] = ???

    /** Returns a lexicographic-order comparator with a function that
     *  extracts a key to be compared with the given Comparator.
     */
    @stub
    val Comparator[T]: default[U] = ???
...
object Comparator {
    ...
    @stub
    def comparing[T, U <: Comparable[_ >: U]](keyExtractor: Function[_ >: T, _ <: U]): Comparator[T] = ???
     ...
     */
    @stub
    def naturalOrder[T <: Comparable[_ >: T]](): Comparator[T] = ???
     ...
     */
    @stub
    def reverseOrder[T <: Comparable[_ >: T]](): Comparator[T] = ???
}
